...
typedef struct {
	...
	int *global_res;			//< Résultat global
} message_t;
...
void * moyenneTableau (void * arg) {
	...
	*(msg->global_res) /= (msg->stop - msg->start);
...
void * maxTableau (void * arg) {
	...
	for( int i = msg->start ; i < msg->stop ; i++) {
		...
		if( msg->tab[i]  >=  *(msg->global_res) ) *(msg->global_res) = msg->tab[i];
	}
...
void programmePrincipal (const arg_t arg) {
	...
	int global_res = 0;
	if( arg.code  ==  3 ) global_res = INT_MAX;
	...
	for( int i = 0 ; i < arg.nbThreads ; i++) {
		...
		tab_msg[i].res   = 0;
		tab_msg[i].global_res = &global_res;
	}
	...
	for( int i = 0 ; i < arg.nbThreads ; i++ )
		printf("msg[%d].global_res = %d\n", i, *(tab_msg[i].global_res));
